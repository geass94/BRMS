...
<table class="table table-hover">
	...
	<tbody>
		<?php
			$query = $MySQL->query("SELECT * FROM users WHERE role != 'ROLE_USER' AND plan != '' ORDER BY id DESC");
		...
		<tr id="users-<?=$row['id']?>">
			<td><?=$row['id']?></td>
			<td><?=$row['username']?></td>
			<td><?=plan($row['plan'],$row['plan'])['text']?></td>
			<td><?=role($row['role'],$row['role'])['text']?></td>
			<td><?=$row['reg_date']?></td>
			<td><?=$row['reg_ip']?></td>
			<td><?php if($row['approved']){ echo "<p class='p bg-success'>გააქტიურებულია</p>"; }else{ echo "<p class='p bg-warning'><a href='/admin.php?mod=companies&amp;cat=edit&amp;id=".$row['id']."'>გასააქტიურებელია</a></p>"; } ?></td>
			<td>
