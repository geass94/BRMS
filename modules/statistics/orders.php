...
$total = mysqli_num_rows($MySQL->query("SELECT id FROM history WHERE module = 'orders' $date_filter"));
?>
    ...
    <div class="form-group">
        <label class="sr-only" for="ro-date"><?=$lang['end_date']?></label>
            <select class="form-control" name="display_count">
                ...
                <option value="100">100 <?=$lang['']?></option>
                <option value="500">500 <?=$lang['']?></option>
            </select>
    </div>
...
<div class="table-responsive">
    <table class="table table-bordered table-hover">
        <thead><th><?=$lang['table_s']?> ID</th><th><?=$lang['name']?></th><th><?=$lang['price']?></th><th><?=$lang['quantity']?></th><th><?=$lang['date']?></th></thead>
        <tbody>
        <?php
                $page = 1; $offset = 0;
...
$export_data = "<table border='1'><thead><th>".$lang['table_s']." ID</th><th>".$lang['name']."</th><th>".$lang['price']."</th><th>".$lang['quantity']."</th><th>".$lang['date']."</th></thead><tbody>";
        	$query = $MySQL->query("SELECT * FROM history WHERE module = 'orders' $date_filter ORDER BY date DESC LIMIT $display_count OFFSET $offset");
			while($row = mysqli_fetch_array($query)){
				$history = get_history($row['array']);
        ?>
        	<tr>
        		<td>
        			<?php  echo cout($history['table_id']); ?>
        		</td>
        		<td>
        			<?php  echo cout($history['menu_item']); ?>
        		</td>
        		<td>
        			<?php  echo cout($history['sell_price']); ?>
        		</td>
        		<td>
        			<?php  echo cout($history['quantity']); ?>
        		</td>
        		<td>
        			<?php  echo cout($history['time']); ?>
        		</td>
        	</tr>
        	<?php 
$export_data .= "<tr><td>".(cout($history['table_id']))."</td><td>".(cout($history['menu_item']))."</td><td>".(cout($history['sell_price']))."</td><td>".(cout($history['quantity']))."</td><td>".(cout($history['time']))."</td></tr>";
        	}
